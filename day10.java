public class day10 {
    ...
    public static void main(String[] args) {
        ...
        if (notFound) {
            reset();
            nextStep(animalRow+1, animalCol, matrix[animalRow][animalCol], "S");
        }
        if (notFound) {
            reset();
            nextStep(animalRow, animalCol-1, matrix[animalRow][animalCol], "W");
        }
        if (notFound) {
            reset();
            nextStep(animalRow, animalCol-1, matrix[animalRow][animalCol], "E");
        }
    ...
    private static void fillMatrix() {
        ...
        matrix = new String[line[0].length()][line.length];
        for (int i = 0; i < matrix.length; i++) {
            matrix[i] = line[i].split("");
            int index = line[i].indexOf("S");
            if(index >= 0) {
                animalRow = i;
                animalCol = index;
            }
